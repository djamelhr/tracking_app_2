...
import { countPorts } from "../../redux/actions/terminalsActions";
...
const Table = () => {
  ...
  const { PortsNumber } = useSelector((state: RootState) => state.terminal);
  ...
  const fetchPorts = async (page: number) => {
    const res = await fetch(`${proxy}/v1/ports/page/${page}/per_page/10`);
  ...
  const nextPage = () => {
    setNumberOfPorts(page * 10 + data.ports.length);
    setPage((old) => old + 1);
    if (page + 1 === PortsNumber) {
      setHasMore(true);
    }
    ...
    <div>
              ...
              <tbody className="bg-white">
                {data.ports.map((port: any) => (
                  <tr key={port.id} className="whitespace-nowrap">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-base text-gray-500">{port.city}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="flex p-4">
        ...
        <button
          ...
          disabled={hasMore || PortsNumber === 0}
        >
          Next Page
        </button>
